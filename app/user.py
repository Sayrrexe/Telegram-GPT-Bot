...
async def start_command(message: types.Message):
    ...
    if args:
        ...
        paths = await generate_image_gpt(response=f'Сгенерируй промт изображения, которое передает [основные черты и настроение, например, спокойствие, динамичность, романтику]. Добавь в него такие детали, как [детали о персонаже, окружении, или цветах]. Пусть изображение выглядит [стиль, например, реалистично, абстрактно или фантазийно], и создаёт впечатление [впечатление, например, таинственности, уюта, мощи]. Вот конкретные детали, которые нужно включить: {result}. Оставь только промт и переведи его на английский!')
